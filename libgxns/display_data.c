...
/*   Updated: 2018/04/10 12:21:32 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_data_vec2(t_win *win, char *name, t_vec2 data, int y)
{
	...
	mlx_string_put(win->mlx, win->win, 170, y, 0xFFFFFF,
...
void	display_data_vec3(t_win *win, char *name, t_vec3 data, int y)
{
	...
	mlx_string_put(win->mlx, win->win, 170, y, 0xFFFFFF,
			(tmp = ft_itoa(rint(data.y))));
	free(tmp);
	mlx_string_put(win->mlx, win->win, 220, y, 0xFFFFFF, ",");
	mlx_string_put(win->mlx, win->win, 240, y, 0xFFFFFF,
