...
void					put_px(float2 px, uchar4 color, __global uchar4 *buf, int2 img_size)
{
	...
	tmp = (float4)(fmin(color.r * a + buf_tmp.r * (1 - a), 255),
				   fmin(color.g * a + buf_tmp.g * (1 - a), 255),
				   fmin(color.b * a + buf_tmp.b * (1 - a), 255), 0);
						  ...
						  )
{
	...
	{
		if (v1.w <= 0.01 && v2.w <= 0.01)
			return ;
		else if (v1.w <= 0.01)
			v1 = mix(v2, v1, (float)(1 - (v1.w - 0.01) / (v1.w - v2.w)) * sign(v1.w));
		else if (v2.w <= 0.01)
			v2 = mix(v1, v2, (float)(1 - (0.01 - v2.w) / (v1.w - v2.w)) * sign(v2.w));
	}
	...
	px = v1;
	while (distance(v1.xy, v2.xy) > distance(v1.xy, px.xy) - 0.2)
	{
		color = choose_color(px.z);
		if (antialiasing == 1)
		{
			if (fabs(unit.x) == 1)
			{
		...
		else if (antialiasing == 2)
		{
			...
			{
				offset = fmod(px.y, 1);
				put_px(px.xy + (float2)(0, -1), (uchar4)(color.rgb, 0xFF * offset), buf, img_size);
				put_px(px.xy + (float2)(0, 1), (uchar4)(color.rgb, 0xFF * (1 - offset)), buf, img_size);
			}
			else
			{
				offset = fmod(px.x, 1);
				put_px(px.xy + (float2)(-1, 0), (uchar4)(color.rgb, 0xFF * offset), buf, img_size);
				put_px(px.xy + (float2)(1, 0), (uchar4)(color.rgb, 0xFF * (1 - offset)), buf, img_size);
			}
		}
		else
			put_px(px.xy, color, buf, img_size);
