...
/*   Updated: 2018/03/15 05:07:55 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			transform_handler(t_ptr *p)
{
	...
	translate(p->transform, (t_vec3){(p->map->x - 1) / -2.0, (p->map->y - 1) / -2.0, 0.0});
	...
	ft_memcpy(p->perspective, (t_mat4){
			s, 0, 0, 0,
			0, s, 0, 0,
			0, 0, -(p->far) / (p->far - p->near), -(p->far * p->near) / (p->far - p->near),
			0, 0, -1, 0}, sizeof(t_mat4));
}
