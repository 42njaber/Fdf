...
/*   Updated: 2018/04/06 13:24:37 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		move(t_ptr *p)
{
	if (p->keys[123])
	{
		p->dest_pos.x += 0.4 * cos(p->rot.y / 180 * M_PI) / p->zoom;
		p->dest_pos.y += 0.4 * -sin(p->rot.y / 180 * M_PI) / p->zoom;
	}
	if (p->keys[124])
	{
		p->dest_pos.x += -0.4 * cos(p->rot.y / 180 * M_PI) / p->zoom;
	...
	if (p->keys[125])
	{
		p->dest_pos.x += -0.4 * sin(p->rot.y / 180 * M_PI) / p->zoom;
		p->dest_pos.y += -0.4 * cos(p->rot.y / 180 * M_PI) / p->zoom;
	}
	if (p->keys[126])
	{
		p->dest_pos.x += 0.4 * sin(p->rot.y / 180 * M_PI) / p->zoom;
